        ...
        {!isLoading && !error && leaderboard.length > 0 ? (
          <div className="divide-y divide-black/10">
            {leaderboard.map((user, index) => (
              <div
                key={user.user_email}
                className="flex items-center justify-between gap-4 py-4 text-[#121212]/75"
              >
                <div className="flex min-w-0 items-center gap-4">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-black/10 text-sm font-bold text-[#121212]">
                    {index + 1}
                  </span>
                ...
                <p className="shrink-0 text-sm font-normal text-[#121212]">
